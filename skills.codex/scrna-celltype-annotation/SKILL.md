...
2. 按 cluster 分组，构建每个 cluster 的 marker 概览表：
   - 过滤显著基因（例如 `p_val_adj < 0.05` 且 `avg_log2FC > 0`）
   - 按 `avg_log2FC` 或其他效应量降序，取前 N 个候选 marker（默认 N=20，可根据上下文调整）
3. 可选：根据 `pct.1` / `pct.2` 等指标，剔除广泛表达的 housekeeping 基因，突出特异性 marker。
